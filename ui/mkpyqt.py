...
if Windows:
    PATH = os.path.join(os.path.dirname(sys.executable), "Lib/site-packages/PySide")
...
if sys.platform.startswith("darwin"):
    ...
    if i > -1:
        PATH = PATH[:i] + "bin"
if Windows:
    # e.g. PYUIC4 = "c/Python27/Scripts/pyside-uic"
    PYUIC4 = os.path.join(os.path.dirname(sys.executable), "Scripts","pyside-uic")
else:
    PYUIC4 = os.path.join(os.path.dirname(sys.executable), "pyside-uic") # e.g. PYUIC4 = "/usr/bin/pyside-uic"

if sys.platform.startswith("darwin"):
    PYUIC4 = os.path.dirname(sys.executable)
    i = PYUIC4.find("Resources")
    if i > -1:
        PYUIC4 = PYUIC4[:i] + "Lib/python2.6/site-packages/PySide/uic/pyuic.py"
PYRCC4 = os.path.join(PATH, "pyside-rcc")
PYLUPDATE4 = os.path.join(PATH, "pyside-lupdate")
LRELEASE = "lrelease"
if Windows:
    PYUIC4 = PYUIC4.replace("/", "\\") + ".exe"
    PYRCC4 = PYRCC4.replace("/", "\\") + ".exe"
    PYLUPDATE4 = PYLUPDATE4.replace("/", "\\") + ".exe"

msg = []
if not os.access(PYUIC4, os.F_OK):
    msg.append("failed to find pyside-uic; tried {0}".format(PYUIC4))
if not os.access(PYRCC4, os.F_OK):
    msg.append("failed to find pyside-rcc; tried {0}".format(PYRCC4))
if not os.access(PYLUPDATE4, os.F_OK):
    msg.append("failed to find pyside-lupdate; tried {0}".format(PYLUPDATE4))
if msg:
    print("\n".join(msg))
...
pyside-lupdate on all .py and .pyw files it encounters, and then runs lrelease
on all .ts files it encounters. It does not use a .pro file so the .ts
files must be created in the first place, e.g., using pyside-lupdate on one
...
NOTE: If any tool fails to run, e.g., pyside-uic, then edit this program and
